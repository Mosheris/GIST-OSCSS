...
st.set_page_config(
    page_title="GIST RSF Survival Risk Calculator",
    page_icon="🧬",
    layout="wide",
)
...
MODEL_CONFIGS = {
    "OS": {
        ...
        "has_marital": True,
        "has_systemic": False,
        "risk_cutoff": 54.344,  # Optimal cutoff from KM curve analysis (Training Set)
    },
    "CSS": {
        ...
        "has_marital": False,
        "has_systemic": True,
        "risk_cutoff": 20.720,  # Optimal cutoff from KM curve analysis (Training Set)
    },
}
...
SITE_OPTIONS = ["Stomach", "Small Intestine"]
...
def encode_race(label: str) -> dict:
    """
    一热编码Race特征。
    White (Race.0) 作为参考类别，返回值不包含Race.0
    """
    if label == "White":
        # Race.0 是参考类别，当其他都为0时隐含为1
        return {"Race.1": 0, "Race.2": 0}
    if label == "Black":
        return {"Race.1": 1, "Race.2": 0}
    if label == "Asian/Pacific Islander":
        return {"Race.1": 0, "Race.2": 1}
    return {"Race.1": 0, "Race.2": 0}


def encode_marital(label: str) -> dict:
    """
    一热编码Marital Status特征。
    Married (Marital.status.0) 作为参考类别，返回值不包含Marital.status.0
    """
    if label is None:
        return {}
    if label == "Married":
        # Marital.status.0 是参考类别，当其他都为0时隐含为1
        return {"Marital.status.1": 0, "Marital.status.2": 0}
    if label == "Single / Unmarried":
        return {"Marital.status.1": 1, "Marital.status.2": 0}
    if label == "Separated / Divorced / Widowed":
        return {"Marital.status.1": 0, "Marital.status.2": 1}
    return {"Marital.status.1": 0, "Marital.status.2": 0}


def encode_site(label: str) -> int:
    """0 for Stomach, 1 for Small Intestine"""
    return 1 if label == "Small Intestine" else 0
...
def build_input_row(
    ...
    # 一热编码特征 - 只更新存在的特征
    race_encoding = encode_race(race_label)
    for key, value in race_encoding.items():
        if key in row:
            row[key] = value

    marital_encoding = encode_marital(marital_label)
    for key, value in marital_encoding.items():
        if key in row:
            row[key] = value

    # 其他特征
    if "Site" in row and site_label is not None:
        row["Site"] = encode_site(site_label)
...
selected_task = st.sidebar.radio("Select Outcome Task", list(MODEL_CONFIGS.keys()))
config = MODEL_CONFIGS[selected_task]
...
cohort = model_pkg.get("cohort", selected_task)
task_features = config.get("features", feature_cols)
...
with st.sidebar:
    ...
    with st.form("patient_data_form"):
        ...
        # Marital Status - 仅OS任务显示
        marital_label = None
        if config.get("has_marital", False):
        ...
        with col_b:
            site_label = st.selectbox("Site", options=SITE_OPTIONS, key=f"{key_prefix}_site")

        col_c, col_d = st.columns(2)
        with col_c:
            liver_label = st.selectbox("Liver Metastasis", options=BINARY_OPTIONS, key=f"{key_prefix}_liver")
        with col_d:
            # Systemic Treatment - 仅CSS任务显示
            systemic_label = None
            if config.get("has_systemic", False):
...
if submit_button:
    ...
    with col1:
        ...
        try:
            ...
            risk_cutoff = config.get("risk_cutoff")

            if risk_cutoff is not None:
                # Risk classification based on KM curve cutoff value
                if risk_score < risk_cutoff:
                ...
                # Fallback to percentile-based classification if cutoff not available
